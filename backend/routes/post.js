...
router.post('/favoritePost', async function (req, res) {
    console.log(req.body)
    PostCol = mango.get().db('test').collection('col')
    names = mango.get().db('app').collection('users')
    var userId = req.session.passport.user  // current user
    if (req.body.favoriteState) {   // if already favorited, means unfavorite
        await PostCol.updateOne({ _id: new ObjectId(req.body.post) }, { $pull: { favoriteArray: userId } }, { upsert: true })
            .then(resul => {
                console.log('post_updated', resul)
            })
            .catch(err => {
        ...
        await names.updateOne({ _id: new ObjectId(userId) }, { $pull: { favoriteArray: req.body.post } }, { upsert: true })
            .then(resul => {
                console.log('updated to', resul)
                res.json(resul)
            })
        ...
        await PostCol.updateOne({ _id: new ObjectId(req.body.post) }, { $push: { favoriteArray: userId } }, { upsert: true })
        .then(resul => {
            console.log('post_updated', resul)
        })
        .catch(err => {
        ...
        await names.updateOne({ _id: new ObjectId(userId) }, { $push: { favoriteArray: req.body.post } }, { upsert: true })
            .then(resul => {
                console.log('updated to', resul)
                res.json(resul)
            })
...
router.post('/favoriteStatus', function (req, res) {
    names = mango.get().db('app').collection('users')
    var userId = req.session.passport.user  // current user
    names.findOne({ _id: new ObjectId(userId) }, { projection: { favoriteArray: 1 } })
        .then(resul => {
            console.log('favoritestatus', resul)
            res.json(resul)
        })
})
